...
docker run -it --rm \
    ...
    -c "mvn --global-settings /root/.m2/settings.xml --batch-mode install -DskipTests; cp /usr/src/mymaven/param-app/src/docker/build-serv-efluid/standalone-with-postgres/* /usr/src/mymaven/param-app/target"
